...
conn['Route Mode'] = {
    'main': [
        [{'node': 'Handle Tool Calls', 'type': 'main', 'index': 0}],
        [
            {'node': 'Persist Transcript Supabase', 'type': 'main', 'index': 0},
            {'node': 'Call AI Chat Webhook', 'type': 'main', 'index': 0},
        ],
    ]
}

# Keep persist for logging only; do not feed AI from persist output.
conn['Persist Transcript Supabase'] = {'main': [[{'node': 'Prepare Agent Input', 'type': 'main', 'index': 0}]]}
...
print(json.dumps(wf2.get('connections', {}).get('Route Mode', {}), ensure_ascii=False))
