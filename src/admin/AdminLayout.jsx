...
const NAV_HELP = {
  ar: {
    dashboard: 'نظرة سريعة على الأداء والحالة العامة.',
    leads: 'إدارة العملاء المحتملين ومتابعة حالتهم.',
    bookings: 'مراجعة الطلبات والمواعيد القادمة.',
    blog: 'إنشاء وتحرير ونشر مقالات المدونة.',
    caseStudies: 'إدارة دراسات الحالة وقصص النجاح.',
    faq: 'تحديث الأسئلة الشائعة وإجاباتها.',
    services: 'إدارة بطاقات الخدمات وأسعارها ووصفها.',
    subscribers: 'متابعة المشتركين والنشرات البريدية.',
    trendIntelligence: 'رصد الاتجاهات العامة في المحتوى والسوق.',
    competitorIntel: 'تحليل محتوى المنافسين وتوليد أفكار قابلة للنشر.',
    analytics: 'مؤشرات الأداء والتقارير الرقمية.',
    settings: 'إعدادات النظام والصلاحيات العامة.',
  },
  en: {
    dashboard: 'Quick overview of health and performance.',
    leads: 'Manage leads and track their pipeline status.',
    bookings: 'Review requests and upcoming appointments.',
    blog: 'Create, edit, and publish blog content.',
    caseStudies: 'Manage case studies and success stories.',
    faq: 'Maintain FAQ questions and answers.',
    services: 'Manage service cards, pricing, and descriptions.',
    subscribers: 'Track subscribers and newsletter activity.',
    trendIntelligence: 'Monitor broad market and content trends.',
    competitorIntel: 'Analyze competitor content and generate publishable ideas.',
    analytics: 'View KPIs and reporting metrics.',
    settings: 'Configure global system settings and permissions.',
  },
}

const AUTO_HINT_PREFIX = {
  ar: {
    action: 'إجراء',
    field: 'حقل',
    section: 'قسم',
...
function SidebarContent({ collapsed, onToggle, lang, isDark, profile, user }) {
  const t = L[lang] || L.en
  const help = NAV_HELP[lang] || NAV_HELP.en
  const isRTL = lang === 'ar'
...
function Topbar({ lang, toggleLang, isDark, toggleTheme, user, profile, onSignOut, onMobileMenu }) {
  ...
  const pageHint = useMemo(() => {
    ...
    const matched =
      ...
      allItems[0]
    const hints = NAV_HELP[lang] || NAV_HELP.en
    return hints[matched.labelKey] || ''
...
export default function AdminLayout() {
  ...
  useEffect(() => {
    const labels = AUTO_HINT_PREFIX[lang] || AUTO_HINT_PREFIX.en

    const applyAutoHints = () => {
      const content = document.querySelector('main')
      if (!content) return

      // Headings: use nearby text as a quick explanation when available.
      content.querySelectorAll('h1, h2, h3').forEach((el) => {
        if (el.getAttribute('title')) return
        const heading = (el.textContent || '').trim()
        if (!heading) return
        const sibling = el.nextElementSibling
        const siblingText = sibling ? (sibling.textContent || '').trim() : ''
        el.setAttribute('title', siblingText || `${labels.section}: ${heading}`)
      })

      // Buttons/links: ensure every clickable control has hover guidance.
      content.querySelectorAll('button, a[role="button"], [data-admin-help]').forEach((el) => {
        if (el.getAttribute('title')) return
        const explicit = (el.getAttribute('data-admin-help') || '').trim()
        if (explicit) {
        ...
        if (!text) return
        el.setAttribute('title', `${labels.action}: ${text}`)
      })

      // Form fields: add fallback hints from aria-label / placeholder.
      content.querySelectorAll('input, textarea, select').forEach((el) => {
        ...
        if (hint) {
          ...
          el.setAttribute('title', labels.field)
        }
      })
    }

    applyAutoHints()
    const observer = new MutationObserver(applyAutoHints)
    const content = document.querySelector('main')
    if (content) {
